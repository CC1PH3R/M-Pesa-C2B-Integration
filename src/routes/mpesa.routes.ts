...
// Health check
router.get('/health', (req: Request, res: Response) => mpesaController.health(req, res));
...
router.get('/debug-config', (_req: Request, res: Response) => {
  res.json({
    consumerKey: {
      ...
      last10: mpesaConfig.consumerKey?.substring((mpesaConfig.consumerKey?.length ?? 0) - 10),
    ...
    consumerSecret: {
      ...
      last10: mpesaConfig.consumerSecret?.substring((mpesaConfig.consumerSecret?.length ?? 0) - 10),
...
// Test authentication
router.get('/test-auth', (req: Request, res: Response) => mpesaController.testAuth(req, res));

// Register C2B URLs with M-Pesa
router.post('/register', (req: Request, res: Response) => mpesaController.registerUrls(req, res));
...
router.post('/simulate', (req: Request, res: Response) => mpesaController.simulate(req, res));
